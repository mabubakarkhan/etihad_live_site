function initRenstate() {
    ...
    if ($(".city-carousel").length > 0) {
        var ms2 = new Swiper(".city-carousel .swiper-container", {
            // loop: false — loop mode clones slides so the same projects appear repeated
            loop: false,
            watchOverflow: true,
            grabCursor: true,
            autoHeight: false,
    ...
    if ($(".slideshow-container_wrap").length > 0) {
        ...
        var _heroSlideN = $(".slideshow-container_wrap .swiper-slide").length;
    ...
    // Floating orange button: make it Home (requested)
    $(".to-top_btn span").text("Home");
    $(".to-top_btn i").attr("class", "fa-regular fa-house");
    ...
    var sliders_init2 = $(".price-range-double").not(".listing-deferred-range");
    ...
    // Wishlist is handled globally in `resources/views/layouts/front.blade.php`
    //   scrollnav  ------------------
