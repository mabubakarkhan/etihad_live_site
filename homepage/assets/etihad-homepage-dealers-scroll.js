(function () {
  ...
  var SELECTOR = '.popular-listings--dealers';
  ...
  function updateProgress(section, cards, index) {
    var fill = section.querySelector('.popular-listings__progress span');
  ...
  function unslickIfNeeded(track) {
    if (
      ...
      window.jQuery(track).hasClass('slick-initialized')
    ) {
      window.jQuery(track).slick('unslick');
    }
  }

  function bindCardHover(section) {
    ...
    section.querySelectorAll('.popular-listings__card').forEach(function (card) {
      var image = card.querySelector('.popular-listings__media img');
  ...
  function initDealersSection(section) {
    ...
    var rail = section.querySelector('.popular-listings__rail');
    var track = section.querySelector('.popular-listings__grid');
    var cards = track ? Array.prototype.slice.call(track.querySelectorAll('.popular-listings__card')) : [];
    ...
    track.classList.remove('homepage-slick');
    rail.classList.remove('homepage-slick-rail');

    section.dataset.gsapHscrollInit = '1';
    section.classList.add('popular-listings--gsap-scroll');
    ...
    var prev = section.querySelector('[data-dealers-prev]');
    var next = section.querySelector('[data-dealers-next]');
  ...
  function init() {
    ...
    document.querySelectorAll(SELECTOR).forEach(initDealersSection);

    return true;
  }

  window.EtihadHomepageDealersScroll = {
    init: init,
    refresh: function () {
      var ScrollTrigger = getScrollTrigger();
      if (ScrollTrigger) {
