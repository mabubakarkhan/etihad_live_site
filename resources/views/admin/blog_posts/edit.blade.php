...
<html lang="en">
    <head>
        ...
        <title>Edit Blog Post | Etihad Admin</title>
        ...
        <div class="min-h-screen flex">
            ...
            <main class="flex-1 bg-gradient-to-br from-slate-100 via-slate-50 to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 transition-colors">
                <header class="px-8 pt-6 pb-4 border-b border-slate-200 dark:border-slate-800/70 flex items-center justify-between flex-wrap gap-3">
                    <div>
                        <h1 class="text-xl md:text-2xl font-semibold tracking-tight">Edit blog post</h1>
                        <p class="text-sm text-slate-500 mt-1">{{ $post->title }}</p>
                    </div>
                    <div class="flex items-center gap-3">
                        @include('admin.partials.theme-toggle')
                        <a href="{{ $post->url() }}" target="_blank" class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium border border-slate-300 dark:border-slate-700">View</a>
                ...
                <section class="px-6 md:px-8 py-6 md:py-8">
                    <form method="POST" action="{{ route('admin.blog-posts.update', $post) }}" enctype="multipart/form-data" class="max-w-4xl space-y-4">
                        @csrf
                        @method('PUT')
                        @if ($errors->any())
                        ...
                        <button type="submit" class="inline-flex items-center justify-center rounded-lg bg-emerald-500 px-4 py-2.5 text-sm font-semibold text-slate-950 shadow shadow-emerald-500/40 hover:bg-emerald-400">Save changes</button>
                    </form>
                </section>
            </main>
        </div>
