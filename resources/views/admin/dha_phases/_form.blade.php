...
<div class="space-y-6 max-w-4xl">
    ...
    <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/80 p-5 shadow-lg space-y-4">
        <h2 class="text-sm font-semibold text-emerald-600 uppercase tracking-wider">Interactive map section</h2>
        <p class="text-xs text-slate-500 dark:text-slate-400">Heading and tagline for the map section on the phase page. Configure the ground overlay below.</p>
        <div>
    ...
    @if(!empty($phase->id))
        ...
        <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/80 p-5 shadow-lg">
            @include('admin.interactive-map._editor', [
                'ownerType' => 'dha-phases',
                'ownerId' => (int) $phase->id,
                'map' => $interactiveMap,
                'standaloneUrl' => route('admin.dha-phases.interactive-map', $phase),
            ])
        </div>
    @endif
