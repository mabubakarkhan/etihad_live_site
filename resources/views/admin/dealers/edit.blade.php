...
<html lang="en">
    <head>
        ...
        <title>Edit Dealer | Etihad Admin</title>
        ...
        <div class="min-h-screen flex">
            ...
            <main class="flex-1 bg-gradient-to-br from-slate-100 via-slate-50 to-slate-100 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 transition-colors">
                <header class="px-8 pt-6 pb-4 border-b border-slate-200 dark:border-slate-800/70 flex items-center justify-between flex-wrap gap-3">
                    <div>
                        <h1 class="text-xl md:text-2xl font-semibold tracking-tight text-slate-900 dark:text-slate-50">Edit dealer</h1>
                        <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">{{ $dealer->name }} · {{ $dealer->properties_count }} properties</p>
                    </div>
                    <div class="flex items-center gap-3 flex-shrink-0">
                        @include('admin.partials.theme-toggle')
                        <a href="{{ route('admin.dealer-listings.index', ['dealer' => $dealer->id]) }}" class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium border border-sky-500/50 text-sky-600 dark:text-sky-400 hover:bg-sky-500/10 transition">View listings ({{ $dealer->properties_count }})</a>
                ...
                <section class="px-6 md:px-8 py-6 md:py-8">
                    <div class="max-w-2xl rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/80 p-6 shadow-lg shadow-slate-200/80 dark:shadow-slate-950/50 transition-colors">
                        ...
                        @endif
                        <form method="POST" action="{{ route('admin.dealers.update', $dealer) }}" enctype="multipart/form-data" class="space-y-4">
                            @csrf
                            @method('PUT')
                            <div class="space-y-1.5">
                                ...
                                @if($dealer->profile_pic)
                                    <div class="mb-2 flex items-center gap-3 flex-wrap">
                                        <img src="{{ asset('storage/' . $dealer->profile_pic) }}" alt="" class="h-16 w-16 rounded-full object-cover border border-slate-300 dark:border-slate-700" />
                                        <label class="inline-flex items-center gap-1.5 text-xs text-rose-600 dark:text-rose-400 cursor-pointer"><input type="checkbox" name="remove_profile_pic" value="1" class="rounded border-slate-400" /> Remove photo</label>
                                    </div>
                                @endif
                            ...
                            <div class="space-y-1.5">
                                ...
                                <input id="name" name="name" type="text" value="{{ old('name', $dealer->name) }}" required class="block w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950/60 px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100" />
                            </div>
                            <div class="space-y-1.5">
                                <label for="slug" class="block text-sm text-slate-700 dark:text-slate-300">Slug</label>
                                <input id="slug" name="slug" type="text" value="{{ old('slug', $dealer->slug) }}" placeholder="url-friendly-id" class="block w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950/60 px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100" />
                            ...
                            <div class="space-y-1.5">
                                ...
                                <select id="status" name="status" class="block w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950/60 px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100">
                                    <option value="active" {{ old('status', $dealer->status ?? 'active') === 'active' ? 'selected' : '' }}>Active</option>
                                    <option value="inactive" {{ old('status', $dealer->status ?? 'active') === 'inactive' ? 'selected' : '' }}>Inactive</option>
                                </select>
                            ...
                            <div class="space-y-1.5">
                                <label class="inline-flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300 cursor-pointer">
                                    <input type="checkbox" name="show_homepage" value="1" {{ old('show_homepage', $dealer->show_homepage) ? 'checked' : '' }} class="rounded border-slate-300 dark:border-slate-700 text-emerald-500 focus:ring-emerald-500">
                                    <span>Show on homepage popular agents section</span>
                                </label>
                                <p class="text-xs text-slate-500 dark:text-slate-400">When checked, this dealer can appear in homepage popular agents (max 4 shown).</p>
                            </div>
                            <div class="space-y-1.5">
                                <label class="inline-flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300 cursor-pointer">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="space-y-1.5">
                                    ...
                                    <input id="email" name="email" type="email" value="{{ old('email', $dealer->email) }}" class="block w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950/60 px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100" />
                                </div>
                                <div class="space-y-1.5">
                                    <label for="phone" class="block text-sm text-slate-700 dark:text-slate-300">Phone</label>
                                    <input id="phone" name="phone" type="text" value="{{ old('phone', $dealer->phone) }}" class="block w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950/60 px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100" />
                                </div>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="space-y-1.5">
                                    <label for="whatsapp" class="block text-sm text-slate-700 dark:text-slate-300">WhatsApp</label>
                                    <input id="whatsapp" name="whatsapp" type="text" value="{{ old('whatsapp', $dealer->whatsapp) }}" class="block w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950/60 px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100" />
                                </div>
                                <div class="space-y-1.5">
                                    <label for="mobile" class="block text-sm text-slate-700 dark:text-slate-300">Mobile</label>
                                    <input id="mobile" name="mobile" type="text" value="{{ old('mobile', $dealer->mobile) }}" class="block w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950/60 px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100" />
                                </div>
                            </div>
                            <div class="space-y-1.5">
                                <label for="info_detail" class="block text-sm text-slate-700 dark:text-slate-300">Short description (team page)</label>
                                <textarea id="info_detail" name="info_detail" rows="3" class="block w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950/60 px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100" placeholder="Brief bio or description shown on the Our Team page.">{{ old('info_detail', $dealer->info_detail) }}</textarea>
                            ...
                            <div class="space-y-1.5">
                                ...
                                <textarea id="address" name="address" rows="2" class="block w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950/60 px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100">{{ old('address', $dealer->address) }}</textarea>
                            </div>
                            @php
                                $states = $states ?? collect();
                                $currentState = old('state', $dealer->state ?? 'Punjab');
                                $currentCity = old('city', $dealer->city ?? 'Lahore');
                            ...
                            <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/80 p-5 shadow-lg transition-colors mt-6">
                                ...
                                <div class="mb-3 flex items-center gap-3 flex-wrap">
                                    <img src="{{ asset('storage/' . $dealer->banner_image) }}" alt="Banner" class="max-h-24 rounded-lg border border-slate-200 dark:border-slate-600 object-cover" />
                                    <label class="inline-flex items-center gap-1.5 text-xs text-rose-600 dark:text-rose-400 cursor-pointer"><input type="checkbox" name="remove_banner_image" value="1" class="rounded border-slate-400" /> Remove banner</label>
                                </div>
                                @endif
                            ...
                            <div class="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/80 p-5 shadow-lg transition-colors mt-6">
                                ...
                                <div class="space-y-4">
                                    <div class="space-y-1.5">
                                        ...
                                        <input id="meta_title" name="meta_title" type="text" value="{{ old('meta_title', $dealer->meta_title) }}" class="block w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950/60 px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100" placeholder="Optional" />
                                    </div>
                                    <div class="space-y-1.5">
                                        <label for="meta_description" class="block text-sm text-slate-700 dark:text-slate-300">Meta description</label>
                                        <textarea id="meta_description" name="meta_description" rows="2" class="block w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950/60 px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100" placeholder="Optional">{{ old('meta_description', $dealer->meta_description) }}</textarea>
                                    </div>
                                    <div class="space-y-1.5">
                                        <label for="meta_keywords" class="block text-sm text-slate-700 dark:text-slate-300">Meta keywords</label>
                                        <input id="meta_keywords" name="meta_keywords" type="text" value="{{ old('meta_keywords', $dealer->meta_keywords) }}" class="block w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950/60 px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100" placeholder="Comma separated, optional" />
                                    </div>
                                    <div class="space-y-1.5">
                                        <label for="canonical_url" class="block text-sm text-slate-700 dark:text-slate-300">Canonical URL</label>
                                        <input id="canonical_url" name="canonical_url" type="text" value="{{ old('canonical_url', $dealer->canonical_url) }}" class="block w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-950/60 px-3 py-2.5 text-sm text-slate-900 dark:text-slate-100" placeholder="Optional" />
                                    </div>
                                </div>
                            </div>
                            <button type="submit" class="inline-flex items-center justify-center rounded-lg bg-emerald-500 px-4 py-2.5 text-sm font-semibold text-slate-950 shadow shadow-emerald-500/40 hover:bg-emerald-400 transition mt-6">Update dealer</button>
                        </form>
                    </div>
                </section>
            </main>
